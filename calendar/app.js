...
/*
  Helpers that allow other scripts (including the browser console)
  to work together with the calendar schedule.

  - `getGroceryListFromSchedule()` will fetch recipe ingredients for each
    scheduled item (via the shared `RecipeScraper` utility) and return an
    aggregated list.
  - `addRecipeToDate()` can be used to schedule a recipe by name (without drag/drop).
*/

async function getGroceryListFromSchedule() {
  if (typeof RecipeScraper === 'undefined' || !RecipeScraper.getGroceryListForRecipeName) {
    throw new Error('RecipeScraper is not loaded. Make sure recipeScraper.js is included before app.js.');
  }

  const aggregated = {};

  for (const [, items] of Object.entries(schedule)) {
    for (const item of items) {
      const { ingredients } = await RecipeScraper.getGroceryListForRecipeName(item.title);
      for (const ing of ingredients) {
        ...
        aggregated[key].amount += Number.isFinite(parsed) ? parsed : 0;
      }
    }
  }
...
async function addRecipeToDate(dateKey, recipeName, time = '') {
  if (!dateKey || !recipeName) {
    throw new Error('dateKey and recipeName are required');
  }

  if (typeof RecipeScraper === 'undefined' || !RecipeScraper.getFirstRecipeForName) {
    throw new Error('RecipeScraper is not loaded. Make sure recipeScraper.js is included before app.js.');
  }

  const recipe = await RecipeScraper.getFirstRecipeForName(recipeName);
  if (!recipe) {
    throw new Error(`No recipe found for "${recipeName}"`);
  }

  addEvent(dateKey, {
    recipeId: recipe.uri ?? recipe.label,
    title: recipe.label,
    time,
  });
}

window.GroceryMate = window.GroceryMate || {};
Object.assign(window.GroceryMate, {
  schedule,
  getGroceryListFromSchedule,
  addRecipeToDate,
});
